...
const ChatBox = ({ conversationId }) => {
    ...
    const loadInitialData = async () => {
      try {
        ...
        const conversationResponse = await axios.get(
          `https://localhost:7288/api/Chat/conversation-info/${conversationId}`,
          {
        ...
        const historyResponse = await axios.get(
          `https://localhost:7288/api/Chat/history/${conversationId}`,
          {
            headers: { Authorization: `Bearer ${token}` }
          }
        );
        ...
        if (historyResponse.data) {
          ...
          setHasMore(historyResponse.data.messages.length > 0);
        }
      } catch (error) {
  ...
  // Tách phần event listeners ra một useEffect riêng
  useEffect(() => {
    const user = JSON.parse(localStorage.getItem("user"));
    if (user) {
      setCurrentUser(user);
    ...
    const setupConnection = async () => {
      try {
        ...
        // Thêm các event handler trước khi start connection
        newConnection.on('ReceiveMessage', (message) => {
          if (message.conversationId.toString() === conversationId.toString()) {
            ...
            const newMessage = {
              ...
              senderName: sellerInfo?.sellerName,
              senderAvatar: sellerInfo?.sellerAvatar,
        ...
        // Load data sau khi connection đã được thiết lập
        await loadChatHistory();
        await loadConversationInfo();
      } catch (error) {
        console.error('Error during connection setup:', error);
        setError("Không thể kt nối đến server chat hoặc tham gia cuộc trò chuyện");
      }
    };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://localhost:7288/api/Chat/send',
        formData,
        {
          headers: {
  ...
  const loadConversationInfo = async () => {
    try {
      ...
      setError("Không th tải thông tin cuộc trò chuyện");
    } finally {
  ...
  // Sửa lại phần render message để kiểm tra kỹ hơn
  const renderMessage = (msg) => {
    const isCurrentUser = msg.senderId === currentUser?.userId;
    
    let senderName, senderAvatar;
    if (isCurrentUser) {
      senderName = sellerInfo?.buyer?.name;
      senderAvatar = sellerInfo?.buyer?.avatar;
    } else {
      senderName = sellerInfo?.seller?.name;
    ...
    try {
      ...
      if (response.data) {
        const formattedMessages = response.data.messages.map(msg => {
          ...
          return {
            ...msg,
            senderName: isCurrentUser ? currentUser.fullName : sellerInfo?.sellerName,
            senderAvatar: isCurrentUser ? currentUser.avatar : sellerInfo?.sellerAvatar,
            imageUrl: msg.imageUrl ? getFullImageUrl(msg.imageUrl) : null,
            isCurrentUser
          };
        });
  ...
  useEffect(() => {
    console.log("Full sellerInfo object:", sellerInfo);
    ...
    const checkSellerStatus = () => {
      ...
      if (user && sellerInfo) {
        ...
        // Kiểm tra xem người dùng hiện tại có phải là seller của conversation này không
        const isConversationOwner = sellerInfo.seller && user.userId === sellerInfo.seller.userId;
        
        console.log("Is user a seller?", userIsSeller);
        console.log("Current user ID:", user.userId);
        console.log("Seller ID in conversation:", sellerInfo.seller?.userId);
        console.log("Is conversation owner?", isConversationOwner);
      ...
      {sellerInfo && (
        <div className="px-6 py-4 border-b border-gray-200 bg-white">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-gray-800">
                  {sellerInfo.seller?.name || "Người bán"}
                </h2>
            ...
            {/* Nút tạo đơn hàng */}
            {isSeller && isConversationSeller && (
              <button
                className="px-4 py-2 bg-green-600 text-white rounded-full text-sm font-medium
                  hover:bg-green-700 active:scale-95 hover:shadow-lg transition-all duration-200
                  flex items-center gap-2"
                onClick={() => console.log("Create order clicked")} // Thêm tạm để test
              >
                <svg
